...
// Map legacy achievement icon strings to KenneyIcon names
const ACH_ICON_MAP = {
    Flame: 'power',
    Droplets: 'plus',
    Trophy: 'trophy',
...
export default function Profile({ user, today, totals, streak, xp, unlockedAchievements, onUpdate, onLogWeight, getWeightHistory, getLast7Days }) {
    const { recalculateTDEE } = useStore();
    ...
    return (
        <div className="profile-scroll">
            ...
            <div className="card profile-header-card">
                ...
                <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <span style={{ fontWeight: 900, fontSize: 18 }}>{user.name}</span>
                        <span style={{ fontSize: 9, fontWeight: 900, background: user.role === 'coach' ? 'var(--c-volt)' : 'var(--c-black)', color: user.role === 'coach' ? 'var(--c-black)' : 'var(--c-paper)', padding: '2px 6px', borderRadius: 4, border: '1px solid var(--c-black)' }}>
                            {user.role === 'coach' ? '⚡ COACH' : 'ATHLETE'}
                        </span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
                        <div className="profile-goal-badge">{user.goal.replace('_', ' ').toUpperCase()}</div>
                        <button className="btn" onClick={useStore().toggleCoachRole} style={{ fontSize: 9, padding: '2px 8px', borderRadius: 6, background: 'var(--c-paper)', border: '1.5px solid var(--c-black)' }}>
                            SWITCH TO {user.role === 'coach' ? 'ATHLETE' : 'COACH'}
                        </button>
                    </div>
                </div>
                <div className="streak-big"><KenneyIcon name="star" size={18} /> {streak}</div>
            </div>

            {/* Level + Body Stats row */}
            <div className="profile-top-grid">
                <div className="card level-card-sm">
                    <div className="level-badge-circle-sm"><KenneyIcon name="power" size={16} /></div>
            ...
            <div className="p-tab-content" style={{ flex: 1, minHeight: 0, overflowY: 'auto', paddingRight: 4, display: 'flex', flexDirection: 'column', gap: 8 }}>
                {tab === 'overview' && (
                    <>
                        {/* Quick Stats */}
                        ...
                        {editing ? (
                            <div className="card" style={{ padding: 12 }}>
                                ...
                                <div className="edit-row"><span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><KenneyIcon name="star" size={14} /> Calories</span>
                                ...
                                <div className="edit-row"><span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><KenneyIcon name="plus" size={14} /> Water (L)</span>
                                ...
                                <div className="edit-row"><span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><KenneyIcon name="food" size={14} /> Protein</span>
                                    <input className="modal-input edit-input" type="number" value={editVals.p} onChange={e => setEditVals({ ...editVals, p: e.target.value })} /></div>
                                <div className="edit-row"><span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><KenneyIcon name="food" size={14} /> Carbs</span>
                                    <input className="modal-input edit-input" type="number" value={editVals.c} onChange={e => setEditVals({ ...editVals, c: e.target.value })} /></div>
                                <div className="edit-row"><span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><KenneyIcon name="food" size={14} /> Fats</span>
                            ...
                            <div className="card goals-card" onClick={startEdit}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                                    ...
                                    <span style={{ fontSize: 12, fontWeight: 800, color: 'var(--c-red)', display: 'flex', alignItems: 'center', gap: 4 }}>EDIT <KenneyIcon name="gear" size={12} /></span>
                                </div>
                                <div className="goal-row"><span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><KenneyIcon name="star" size={14} /> Calories</span><span style={{ fontWeight: 800 }}>{user.calorieTarget} kcal</span></div>
                                <div className="goal-row"><span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><KenneyIcon name="food" size={14} /> Protein</span><span style={{ fontWeight: 800 }}>{user.macros.p}g</span></div>
                                <div className="goal-row"><span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><KenneyIcon name="food" size={14} /> Carbs</span><span style={{ fontWeight: 800 }}>{user.macros.c}g</span></div>
                                <div className="goal-row"><span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><KenneyIcon name="food" size={14} /> Fats</span><span style={{ fontWeight: 800 }}>{user.macros.f}g</span></div>
                                <div className="goal-row"><span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><KenneyIcon name="plus" size={14} /> Water</span><span style={{ fontWeight: 800 }}>{user.waterGoal}L</span></div>
                            </div>
                        )}
                    </>
                )}
                ...
                {tab === 'stats' && (
                    <>
                        ...
                        <div className="card" style={{ padding: 12 }}>
                            ...
                            {[
                                { label: 'Protein', val: totals.p, goal: user.macros.p, color: '#FFD700', icon: <KenneyIcon name="food" size={16} /> },
                                { label: 'Carbs', val: totals.c, goal: user.macros.c, color: '#00BFFF', icon: <KenneyIcon name="food" size={16} /> },
                                { label: 'Fats', val: totals.f, goal: user.macros.f, color: '#FF4500', icon: <KenneyIcon name="food" size={16} /> },
                            ].map(m => (
                        ...
                        {weightHist.length > 0 ? (
                            ...
                            <div className="card" style={{ textAlign: 'center', padding: 20 }}>
                                <div style={{ marginBottom: 8, opacity: 0.3 }}><KenneyIcon name="scale" size={40} /></div>
                ...
                {tab === 'achievements' && (
                    <div className="card" style={{ padding: 12 }}>
                        <div className="text-label" style={{ marginBottom: 8, display: 'flex', alignItems: 'center', gap: 4 }}>
                            <KenneyIcon name="trophy" size={12} /> ACHIEVEMENTS ({unlocked.length}/{ACHIEVEMENTS.length})
                        </div>
                        <div className="achievements-grid">
                            {ACHIEVEMENTS.map(a => {
                                const isUnlocked = unlocked.includes(a.id);
                                const kenneyName = ACH_ICON_MAP[a.icon] || 'star';
                                return (
                                    <div key={a.id} className={`achievement-item ${isUnlocked ? 'unlocked' : 'locked'}`} title={a.desc}>
                                        <div className="achievement-icon">
                                            {isUnlocked ? <KenneyIcon name={kenneyName} size={18} /> : <KenneyIcon name="lock" size={14} />}
                                        </div>
